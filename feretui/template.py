...
class Template:
    ...
    def get_template(self, name, lang='en', tostring=True,
        ...
        if tostring:
            return self.render_template(tmpl, self.known[name]['encoding'])

        return tmpl

    def render_template(self, template, encoding):
        res = html.tostring(template, encoding=encoding)
    ...
    def load_template(self, element, ignore_missing_extend=False):
        ...
        encoding = element.attrib.pop('encoding', None)
        ...
        if encoding:
            self.known[name]['encoding'] = encoding

        if tag:
            self.known[name]['tag'] = tag
